...
class SearchEngine:
    def __init__(self, corpus):
        ...
        #Initialisation des données pour la matrice TF
        doc_ids, mot_ids, freq = [], [], []
        ...
        for doc_id, doc in corpus.id2doc.items():
            ...
            for mot, count in compteur.items():
                ...
                freq.append(count)
        #Matrice TF
        #Si erreur de chargement de la matrice TF, mettre/enlever le commentaire de la ligne suivante
    ...
    def search(self, requete, n=5, auteur=None, date_debut=None, date_fin=None):
        #Nettoyage et vecteur pour la requête
        requete = self.corpus.nettoyer_texte(requete).split()
        ...
        #Conversion les dates en objets datetime pour comparaison
        if date_debut:
            date_debut = datetime.strptime(date_debut, "%Y/%m/%d")
        if date_fin:
            date_fin = datetime.strptime(date_fin, "%Y/%m/%d")

        #Utilisation de tqdm classique avec file=sys.stdout
        for doc_id, doc in tqdm(self.corpus.id2doc.items(), desc="Calcul des similarités", unit="document", file=sys.stdout):
            
            #Filtrage par auteur ou date si spécifié
            if auteur and auteur.lower() not in doc['author'].lower():
                continue

            if date_debut or date_fin:
                try:
                    doc_date = datetime.strptime(doc['date'], "%Y/%m/%d")
                    if date_debut and doc_date < date_debut:
                        continue
                    if date_fin and doc_date > date_fin:
                        continue
                except ValueError:
                    #Ignorer les documents avec des dates non valides
                    continue
            
            #Initialisation des vecteurs document
            texte_nettoye = self.corpus.nettoyer_texte(doc['text']).split()
        ...
        for doc_id, score in similarites[:n]:
            ...
            resultats.append([doc['title'], doc['author'], doc['date'], score])

        #Conversion en DataFrame
        df_resultats = pd.DataFrame(resultats, columns=["Titre", "Auteur", "Date", "Score de Similarité"])
        return df_resultats
